...
interface InlineImagePreviewFromUrlProps {
...
export function InlineImagePreviewFromUrl({
  selectedFile,
  allFiles,
  selectedIndex,
  onIndexChange,
  onClose,
}: InlineImagePreviewFromUrlProps) {
    ...
    <div className="mt-4 space-y-3 rounded-lg border-2 border-primary/20 bg-primary/5 p-4">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2 flex-1 min-w-0">
          <Icons.camera className="h-5 w-5 text-primary shrink-0" />
          <div className="flex-1 min-w-0">
            <h4 className="text-sm font-bold text-foreground">Vista Previa de Fotografía</h4>
            <p className="text-xs text-primary font-medium truncate">{selectedFile.originalName}</p>
          </div>
        </div>
        <Button
          type="button"
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex-1 text-center">
            <p className="text-xs font-medium text-foreground">
              Fotografía {selectedIndex + 1} de {allFiles.length}
            </p>
          </div>
          <Button
            type="button"
